...
#依据最终link, 下载img
def download_img(link, img_name_prefix):
...
def get_img_links(theme_link):
    ...
    first = True
    print 'Download images in: %s' % img_download_dir
    ...
    for i in range(1, string.atoi(max_links_num) + 1):  #遍历本主题所有图片link
        ...
        download_img(img_download_link, img_name_prefix)
        '''
        计算next的link
...
def get_zol_image():
    ...
    for theme_link in get_theme_links():
        get_img_links(theme_link)
